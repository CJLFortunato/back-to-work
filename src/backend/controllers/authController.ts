...
export async function registerUser (formData: FormData) {
    ...
    try {
        ...
        userService.updateUser(newUser.id, {...newUser, lastConnexion: new Date(Date.now())});
        canRedirect = true;
    } catch (error) {
        console.error(error);
    }
...
export async function loginUser (formData: FormData) {
    ...
    try {
        ...
        userService.updateUser(user.id, {...user, lastConnexion: new Date(Date.now())});
        canRedirect = true;
    } catch (error) {
        console.error(error);
    }
...
export async function logoutUser () {
    ...
    if (canRedirect) return redirect('/');
}